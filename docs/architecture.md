...
If it **awaits** the Cursor agent webhook before responding, AgentMail/Svix treats the slow response as a failed delivery and **redelivers** (same `svix-id` / `event_id`). Long agent wakes then cause the agent to answer the same inbound repeatedly.
...
- Extracts sender from `message.from` (`Name <user@example.com>` or bare email); matches `ALLOWLIST` case-insensitively.
